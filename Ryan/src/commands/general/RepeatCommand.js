...
const RepeatCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt => opt.setName('reply_to').setDescription('Text to search for and reply to (searches last 20 messages)'))
    .addStringOption(opt => opt.setName('reactions').setDescription('Emojis to add (space separated)'))
    .addBooleanOption(opt => opt.setName('allow_ping').setDescription('Allow mentions in the message? (Default: False)'))
    .addIntegerOption(opt => opt.setName('search_limit').setDescription('How many messages to search? (Default: 20, Max: 100)').setMinValue(1).setMaxValue(100)),
  ...
  execute: async (interaction) => {
    ...
    if (!messageContent && !reactionsStr) {
      return interaction.reply({ content: '❌ You must provide either a message or reactions.', flags: MessageFlags.Ephemeral });
    }
    ...
    try {
      ...
      if (replyToQuery) {
        ...
        targetMessage = messages.find(m =>
          m.content.toLowerCase().includes(replyToQuery.toLowerCase())
        ) || null;
      }
      ...
      if (targetMessage) {
        ...
        if (replyToQuery) {
          if (messageContent) {
            ...
            await interaction.followUp({ content: '⚠️ Target message not found. Sent as new message.', flags: MessageFlags.Ephemeral });
          } else {
